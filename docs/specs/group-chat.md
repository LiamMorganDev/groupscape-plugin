...
- When the typed line is `!gs`-prefixed **and** `ChatboxInput.chatType` is **Public or Clan**, call `.consume()` to suppress the send and reroute the text to GroupScape instead.
- Private and Trade chat types are left untouched — `!gs` in those channels sends as normal chat.
